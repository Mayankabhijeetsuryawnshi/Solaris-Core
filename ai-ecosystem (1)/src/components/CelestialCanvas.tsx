...
export default function CelestialCanvas({
  ...
  useEffect(() => {
    ...
    const getPresetColors = () => {
      const modePreset = colorPreset || (isPhysicsMode ? "hyper-cyan" : isProMode ? "stellar-orbit" : "aurora-emerald");
    ...
    // Instantiate scientific data telemetry columns
    const generateMatrixDrops = () => {
      const colWidth = 55; // wider spacing for clean data logs
      const colCount = Math.floor(width / colWidth) + 1;
      const list = ["0x0A", "He-3", "Orbit", "Node", "F=ma", "IADC", "P_coll", "ψ", "u(t)", "v(t)", "dx/dt", "∇²", "Kepler", "Drag"];
      const drops: MatrixDrop[] = [];
      for (let i = 0; i < colCount; i++) {
        drops.push({
          x: i * colWidth + Math.random() * 4,
          y: Math.random() * -height - 100,
          speed: Math.random() * 0.45 + 0.15, // much slower, controlled lab system telemetry
          size: 7.5, // tiny precise laboratory labels
          chars: Array.from({ length: 6 }, () => list[Math.floor(Math.random() * list.length)])
        });
      }
    ...
    const handleMouseMove = (e: MouseEvent) => {
      mouseRef.current.targetX = e.pageX;
      mouseRef.current.targetY = e.pageY;
    ...
    const handleTouchMove = (e: TouchEvent) => {
      if (e.touches && e.touches.length > 0) {
        mouseRef.current.targetX = e.touches[0].pageX;
        mouseRef.current.targetY = e.touches[0].pageY;
        mouseRef.current.active = true;
        lastMouseMoveTimeRef.current = performance.now();
      }
    };
    ...
    const resizeObserver = new ResizeObserver((entries) => {
      for (const entry of entries) {
        ...
        const rectWidth = parent ? parent.clientWidth : (entry.contentRect.width || window.innerWidth);
        const rectHeight = parent ? parent.clientHeight : (entry.contentRect.height || window.innerHeight);

        width = canvas.width = rectWidth;
        height = canvas.height = rectHeight;
    ...
    const render = () => {
      ...
      // Determine if we are in an idle standby state
      const isIdle = (performance.now() - lastMouseMoveTimeRef.current > 4000) && !mouseRef.current.active;

      // Celestial dark clean transparent clearing
      ctx.fillStyle = isPhysicsMode ? "rgba(2, 5, 18, 0.22)" : isProMode ? "rgba(4, 1, 10, 0.22)" : "rgba(7, 9, 14, 0.22)";
      ...
      // 1. Physical Data Telemetry flow (Clean scientific watermark layer in PRO mode)
      if (isProMode) {
        ...
        for (const d of drops) {
          ...
          for (let i = 0; i < d.chars.length; i++) {
            const charY = d.y - (i * d.size * 2.2);
            if (charY < 0 || charY > height) continue;

            const alphaRatio = (d.chars.length - i) / d.chars.length;
            const alpha = alphaRatio * 0.08; // very faint, non-distracting watermark

            if (i === 0) {
              ctx.fillStyle = `rgba(255, 255, 255, ${alpha * 2.2})`; // White lead glow head
            } else {
            ...
            // Mutate character values for dynamic streams
            if (Math.random() > 0.99) {
      ...
      // 3. Render Stars with custom physical patterns & gravitational fields
      for (const star of stars) {
        star.twinklePhase += star.twinkleSpeed;
        const twinkleAlpha = (isIdle ? 0.12 : 0.22) + (Math.sin(star.twinklePhase) + 1) * (isIdle ? 0.2 : 0.4);

        let targetX = star.baseX;
        let targetY = star.baseY;

        const idx = stars.indexOf(star);
        const timeFactor = isIdle ? performance.now() * 0.00015 : performance.now() * 0.0008;

        if (isPhysicsMode) {
          const row = idx % 3;
          const ratioX = idx / stars.length;
          targetX = (width - 100) * ratioX + 50;
          targetY = height * (0.25 + row * 0.25) + Math.sin((targetX / width) * Math.PI * 4 + timeFactor + row * Math.PI) * 45;
        } else if (isProMode) {
          const cols = 8;
          const colWidth = width / (cols + 1);
          const colIndex = idx % cols;
          const rowIndex = Math.floor(idx / cols);
          const totalRowsForCols = Math.ceil(stars.length / cols);
          targetX = colWidth * (colIndex + 1);
          targetY = (height / (totalRowsForCols + 1)) * (rowIndex + 1) + Math.sin(timeFactor * 1.5 + idx) * 4;
        } else {
          // Standard Consensus Roundtable concentric tracks representing active participants
          const centerX = width / 2;
          const centerY = height / 2;
          const ringIndex = idx % 4;
          const radiusVal = Math.min(width, height) * 0.11 * (ringIndex + 1.2);
          const speedFactor = 0.5 / (ringIndex + 1);
          const positionAngle = (idx * (Math.PI * 2 / 16)) + (isIdle ? performance.now() * 0.00001 : performance.now() * 0.00008) * speedFactor;
          targetX = centerX + Math.cos(positionAngle) * radiusVal;
          targetY = centerY + Math.sin(positionAngle) * radiusVal;
        }

        // Smooth physical controlled snap interpolation (No continuous endless chaotic drift)
        star.x += (targetX - star.x) * 0.035;
        star.y += (targetY - star.y) * 0.035;
