...
import { Copy, Check, RotateCcw, Clock, AlertTriangle, Terminal, Sparkles, MessageSquare } from "lucide-react";
...
export default function ModelCard({ model, response, onRegenerate, history, responseMode, onOpenChat }: ModelCardProps) {
  ...
  const renderMathematicalFormula = (content: string) => {
    ...
    // Format power superscripts, e.g., x^2 or e^x
    processed = processed.replace(/([a-zA-Z0-9_ωθπμσαβγλΔδεφψηητ.()<>]+)\^([a-zA-Z0-9_ωθπμσαβγλΔδεφψηητ.+-\/()<>]+)/g, "$1<sup>$2</sup>");

    // Format subscripts, e.g., v_0 or x_k
    processed = processed.replace(/([a-zA-Z0-9_ωθπμσαβγλΔδεφψηητ.()<>]+)_([a-zA-Z0-9_ωθπμσαβγλΔδεφψηητ.+-\/()<>]+)/g, "$1<sub>$2</sub>");
  ...
  const renderFractions = (text: string): React.ReactNode => {
    ...
    matches.forEach((match, idx) => {
      ...
      if (matchIndex > lastIndex) {
        parts.push(<span key={`t-${idx}`}>{renderMathematicalFormula(text.substring(lastIndex, matchIndex))}</span>);
      }
      ...
      parts.push(
        <span key={`f-${idx}`} className="inline-flex flex-col items-center justify-center align-middle mx-1 bg-slate-950/40 border border-slate-800/50 px-1 rounded-md text-[11px] leading-tight font-serif text-sky-305">
          <span className="border-b border-sky-450/40 pb-0.5 px-1 block text-center min-w-[20px]">{renderMathematicalFormula(numerator)}</span>
          <span className="pt-0.5 px-1 block text-center min-w-[20px]">{renderMathematicalFormula(denominator)}</span>
        </span>
      );
    ...
    if (lastIndex < text.length) {
      parts.push(<span key="tail">{renderMathematicalFormula(text.substring(lastIndex))}</span>);
    }

    return <>{parts}</>;
  };

  const renderFormulasBlock = (text: string): React.ReactNode => {
    const formulaRegex = /(?:\\|\/)?formula\{([\s\S]*?)\}/g;
    const matches = [...text.matchAll(formulaRegex)];
    
    if (matches.length === 0) {
      return renderFractions(text);
    }
    ...
    matches.forEach((match, idx) => {
      const matchIndex = match.index!;
      if (matchIndex > lastIndex) {
        parts.push(<span key={`pre-${idx}`}>{renderFractions(text.substring(lastIndex, matchIndex))}</span>);
      }

      const formulaContent = match[1];
      parts.push(
        <div key={`blk-${idx}`} className="my-3.5 p-4 rounded-xl border border-blue-500/30 bg-slate-950/90 shadow-[0_0_20px_rgba(59,130,246,0.15)] select-all transition-all hover:bg-slate-950 flex flex-col items-center justify-center text-center relative overflow-hidden group">
          <div className="absolute top-2 left-3 text-[7.5px] font-mono text-blue-400 tracking-wider uppercase font-extrabold select-none">
            Mathematical Formula
          </div>
          <div className="text-[14.5px] leading-relaxed text-blue-200 mt-2 font-serif font-semibold tracking-wide">
    ...
    if (lastIndex < text.length) {
      parts.push(<span key="tail-blk">{renderFractions(text.substring(lastIndex))}</span>);
    }
    ...
    return codeParts.map((part, cIdx) => {
      ...
      return boldParts.map((bPart, bIdx) => {
        ...
        return italicParts.map((iPart, iIdx) => {
          ...
          return renderFormulasBlock(iPart);
        });
      });
    });
  };
    ...
    >
      ...
      <div className="flex items-center justify-between pb-3 mb-3 border-b border-gray-800/40 ml-1">
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            ...
            <span className="text-[9px] text-gray-500 uppercase tracking-widest font-mono">Model Node</span>
          </div>
        </div>
          ...
          >
            ...
            {(!history || history.length === 0) && response.status === "idle" && (
              <div className="flex flex-col items-center justify-center text-center text-gray-600 py-12">
                ...
                <p className="text-[10px] font-mono tracking-wider uppercase">No log data available</p>
              </div>
            )}
          ...
          <div ref={scrollContainerRef} className="flex-grow flex flex-col justify-start overflow-hidden">
            {response.status === "idle" && (
              <div className="flex-grow flex flex-col items-center justify-center text-center text-gray-600 py-12">
                <Sparkles className="w-6 h-6 mb-2 opacity-30" />
                <p className="text-[11px] font-mono tracking-wider uppercase pb-1">Awaiting input query</p>
                <p className="text-[9px] text-zinc-550 max-w-[200px] leading-relaxed mx-auto font-mono">Select target models, enter query, and execute.</p>
              </div>
            )}
      ...
      <div className="flex items-center justify-between pt-3 mt-3 border-t border-gray-800/40 ml-1 text-gray-500 text-[10px] font-mono">
        ...
        <div className="flex items-center space-x-1">
          {onOpenChat && (
            <button
              onClick={() => onOpenChat(model.id)}
              className="p-1 px-1.5 rounded bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 border border-amber-500/20 hover:border-amber-500/30 transition-all flex items-center space-x-1 cursor-pointer"
              title="Open dedicated chatbot"
            >
